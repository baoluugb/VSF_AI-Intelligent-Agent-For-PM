...
| 2.3 | **Day-over-day diff**                    | ✅ Done | [sqlite_store.py](src/storage/sqlite_store.py) — `get_daily_diff()` compares against the most-recent prior snapshot (gap-robust) and diffs only meaningful fields (status/assignee/due_date/priority); incremental ingest snapshots only changed entities |
...
| [run_agent.py](src/run_agent.py)                                       | ~180  | Week-6 orchestrator: incremental ingest (snapshots tagged by `--date`) → concerns → grounded report → `output/`; `--reset` for a clean baseline (default accumulates history for the real diff) |
| [run_agent.sh](run_agent.sh)                                           | 73    | One-command wrapper: `run_agent.py --reset` (clean baseline) + V2/V3 verification                                |
...
2. **Concern-Engine precision is prevalence-sensitive — now measured, not estimated.** `test_concern_engine` measures **0.92** on a sampled mix (108 anomalies + 100 normals). `test_precision_full_prevalence` runs on all 856 normals: **recall 1.00**, precision **0.52 overall**, **0.66 on actionable concerns (sev ≥ 3)**, **0.87 on top-priority (sev ≥ 4)**. The overall drop is the `stalled` rule surfacing genuinely-stale normal tasks; chronic backlog (sev 2) is intentionally a de-prioritised _count_, so the SLO is set on the actionable set. The rule is deliberately **not** tightened to require `needs-review` — that would overfit the synthetic labels and hurt real-world recall.
...
4. **Proxy throttling (only when `OPENAI_BASE_URL` points at a proxy).** The default now talks directly to `api.openai.com` and the latest live run succeeded. When a proxy like ckey.vn is configured it can rate-limit bursts (`403`) — mitigated by retry-with-backoff + a deterministic fallback, so a run never crashes.
...
2. ~~**Measure V4 at full prevalence**~~ — **DONE** (`test_precision_full_prevalence`): recall 1.00; precision 0.52 overall / 0.66 actionable (sev ≥ 3) / 0.87 top (sev ≥ 4). SLO set on the actionable set.
